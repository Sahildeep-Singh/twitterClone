...
const _lazyGroqStore = async () => {
  const pkg = await Promise.resolve().then(() => _interopRequireWildcard(require('@sanity/groq-store')));
  const {
    groqStore
...
const _lazyEventSourcePolyfill = async () => {
  const pkg = await Promise.resolve().then(() => _interopRequireWildcard(require('event-source-polyfill')));
  const {
    EventSourcePolyfill
  } = "default" in pkg ? pkg.default : pkg;
